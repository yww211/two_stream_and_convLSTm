...
local testName
if  true then
 -- local RGB_name = 'orderadjust-data_feat_test_RGB_two_video.t7'
   local RGB_name = 'convlstm_rgb-feattwo_video.t7'
  --local RGB_name = 'test3-data_feat_test_RGB_two_video.t7'
   -- training and testing data from UCF101 website
   print(paths.concat(opt.spatFeatDir, RGB_name))
   assert(paths.filep(paths.concat(opt.spatFeatDir, RGB_name)), 'no spatial training feature file found.')
   --data_feat_test_RGB_centerCrop_25f_sp1_part1.t7
   --data_feat_test_FlowMap-TVL1-crop20-1_centerCrop_25f_sp1_part1.t7
   local spaTrainFeatureLabels = torch.load(paths.concat(opt.spatFeatDir, RGB_name))
   spaTrainData = spaTrainFeatureLabels.featMats:permute(1,5,2,3,4)
   testName = spaTrainFeatureLabels.name
     for  key,val in pairs(testName) do
    print('di'..key..'ge name is:'..val)
    print('------>')
  end
   print('spaTrainData de size is :')
   print(spaTrainData:size())
...
----------------------test part----------------
   assert(paths.filep(paths.concat(opt.spatFeatDir, RGB_name)), 'no spatial testing feature file found.')
   local spaTestFeatureLabels = torch.load(paths.concat(opt.spatFeatDir, RGB_name))
   spaTestData = spaTestFeatureLabels.featMats:permute(1,5,2,3,4)
...
if true then 
   ...
   trainData = torch.cat(spaTrainData, tempTrainData, 3)
   testData = torch.cat(spaTestData, tempTestData, 3)

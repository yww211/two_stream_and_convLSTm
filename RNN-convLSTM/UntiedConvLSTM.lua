...
function UntiedConvLSTM:buildCellGateUntied()
   ...
   cellGate:add(backend.SpatialConvolution(self.inputSize, self.outputSize, self.km, self.km, self.stride, self.stride, self.padc, self.padc))
   ...
   if self.step == 1 then
      ...
      if self.train ~= false then
         ...
         output, cell = unpack(recurrentModule:updateOutput{input, prevOutput, prevCell})
          ...
          self.recurrentModule = self:getStepModule(self.step)
...
function UntiedConvLSTM:_updateGradInput(input, gradOutput)
   ...
   if self.gradPrevOutput then
      self._gradOutputs[step] = nn.utils.recursiveCopy(self._gradOutputs[step], self.gradPrevOutput)

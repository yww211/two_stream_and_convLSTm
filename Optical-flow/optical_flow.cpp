...
//Target dataset:hmdb51
//extract optical flow from a floder has no subfloders
...
//string inDir(dirDatabase + "hmdb51_org_split1_train_half1/");
//string outDir(dirDatabase + "hmdb51_org_split1_train_half1-tv/");
// string dirDatabase("/home//Activity-Reco/HMDB-51/");
//string dirDatabase("/media/chih-yao/SSD/dataset/UCF-101/");
string dirDatabase("/home/yww/Activity-Reco/HMDB-51/");
string inDir(dirDatabase + "hmdb51_org_75percent/");
string outDir(dirDatabase + "hmdb51_org-tv_75percent/");
...
int main(int argc, const char* argv[])
{
	...
	cout<< "创建"<<outDir.c_str()<<"..."<<endl;
	if (!opendir(outDir.c_str())) // create the folder if not existed
	{
		if(0==mkdir(outDir.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH))
		{
		cout<< "创建文件夹"<<outDir<<"成功"<<endl;
			}
		else cout<< "创建文件夹"<<outDir<<"失败"<<endl;
	}
	else  cout<< "不存在"<<outDir<<"且创建失败"<<endl;
	...
	if (d)
	{
		while ((dir = readdir(d)) != NULL)
	    {
			...
			{
				string nameOneVideo(dir->d_name);
				nameVideos.push_back(nameOneVideo);
//				printf("%s\n", dir->d_name);
			}
	    }
	    closedir(d);
	}	
	else cout<< "打开文件夹"<<inDir<<"失败"<<endl;

	sort(nameVideos.begin(), nameVideos.end()); // sort by the name
	int numVideoTotal = nameVideos.size();  // 101 classes

	/* Process all the classes */
//	for (int c=60; c<61; c++)
	for (int c=0; c<numVideoTotal; c++)
//	for (int c=numClassTotal-1; c>=0; c--)
	{
		cout << "Current Video: " << c << ". " << nameVideos[c] << endl;
...
//		for (int sv=146; sv<148; sv++)
		...
		//{
			...
			split(nameParse, nameVideos[c], is_any_of("."));

			string pathVideoIn = inDir + nameVideos[c]; // input
			string nameFlow = nameParse[0] + "_flow";
			string pathVideoOut = outDir + nameFlow + "." + nameParse[1]; // output
...
//				cout << "Computation time for " + nameFlow + " (Brox): " << timeSec << " sec" << endl;
		...
		cout << "Computation time for " + nameVideos[c] + " (" + methodOF + "): " << timeSec/60.0 << " min. " << endl;
